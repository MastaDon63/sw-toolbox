...
function fetchAndCacheWithTime(request, options) {
    options = options || {};
    var successResponses = options.successResponses ||
        globalOptions.successResponses;
    ...
    return fetch(request.clone()).then(function(response) {
        ...
        if (request.method === 'GET' && successResponses.test(response.status)) {
            openCache(options).then(function(cache) {
                var resp = newResponse(response.clone(), function(headers) {
                    headers.set('Last-Modified', Date.now().toString());
                    return headers;
                });
                cache.put(request, resp).then(function() {
                    // If any of the options are provided in options.cache then use them.
                    // Do not fallback to the global options for any that are missing
                    // unless they are all missing.
                    var cacheOptions = options.cache || globalOptions.cache;
...
function newResponse(res, headerFn) {
    function cloneHeaders() {
        ...
        for (var kv in res.headers.entries()) {
            if(Object.prototype.hasOwnProperty.call(res.headers.entries(), kv)) {
...
module.exports = {
    ...
    fetchAndCacheWithTime: fetchAndCacheWithTime,
    openCache: openCache,
