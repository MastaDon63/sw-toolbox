...
function cacheFirstWithTiming(request, values, options) {
    options = options || {};
    var cacheOptions = options.cache || globalOptions.cache;
    var cacheQueryOptions = cacheOptions.queryOptions;
    helpers.debug('Strategy: cache first with timings [' + request.url + ']', options);
    return helpers.openCache(options).then(function(cache) {
        return cache.match(request, cacheQueryOptions).then(function(response) {
            ...
            return helpers.fetchAndCacheWithTime(request, options);
        });
    });
}

module.exports = cacheFirstWithTiming;
